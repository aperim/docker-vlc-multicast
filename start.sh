...
SOUT="#transcode{venc=x264{${VLC_X264}},vfilter=canvas{width=200,height=100,aspect=16:9},transcode-threads=${VLC_THREADS},vcodec=h264,fps=${VLC_FPS},threads=${VLC_THREADS},vb=${VLC_BITRATE}}:duplicate{dst='rtp{access=udp,mux=ts,ttl=15,dst=${VLC_MULTICAST_IP},port=${VLC_MULTICAST_PORT},sdp=sap://,group=\"${VLC_SAP_GROUP}\",name=\"${VLC_SAP_NAME}\"}'}"
